...
```
这按道理应该不等的，但是结果确实相等--由于必须给一个可选去包装后再和比较对象进行比较（特别是必须要检测一个可选是不是nil）。上面的例子中不是比较包装了"Howdy"的可选类型，Swift自动的比较里面的包装对象。如果Optional里面包装的对象不是"Howdy"，比较结果就会失败。如果里面没有包装对象，比较也会失败。
